...
const services = [
  { icon: Heart, title: "Matrimony", desc: "Verified profiles, traditional matching, family-first introductions.", to: "/matrimony" },
  { icon: Users, title: "Member Directory", desc: "Connect with families across Coimbatore by area and profession.", to: "/members" },
  { icon: GraduationCap, title: "Scholarships", desc: "Merit and need-based aid for school, college and higher studies.", to: "/scholarships" },
  { icon: Calendar, title: "Events & Meets", desc: "Annual matrimony meets, cultural gatherings and community days.", to: "/events" },
  { icon: Briefcase, title: "Business Network", desc: "A trusted circle of Nadar entrepreneurs, traders and professionals.", to: "/members" },
  { icon: HandHeart, title: "Donations & Welfare", desc: "Healthcare, emergency aid and welfare for community families.", to: "/about" },
];
    ...
    <SiteLayout>
      ...
      <section className="relative overflow-hidden bg-primary-deep text-white">
        ...
        <div className="container-page relative py-20 md:py-28">
          <div className="grid lg:grid-cols-[1.4fr_1fr] gap-12 lg:gap-16 items-center">
            <div className="max-w-3xl">
              ...
              <h1 className="mt-5 font-display text-4xl sm:text-5xl md:text-6xl leading-[1.05]">
                {t.org}
              </h1>
            ...
            <div className="relative mx-auto lg:mx-0 w-full max-w-sm">
              ...
              <div className="absolute -top-3 -left-3 chip !bg-white !text-primary-deep z-10">
                <Sparkles size={12} /> Our Inspiration
              </div>
              <div className="relative overflow-hidden rounded-2xl bg-white/5 ring-1 ring-white/15">
                <img
                  src={kamarajPortrait}
                  alt="Perunthalaivar K. Kamaraj — former Chief Minister of Tamil Nadu, Bharat Ratna"
                  width={257}
                  height={343}
                ...
                <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 via-black/40 to-transparent">
                  ...
                  <div className="text-xs text-white/80 mt-0.5">Former CM of Tamil Nadu · Bharat Ratna · Pride of the Nadar community</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-16 grid grid-cols-3 gap-4 sm:gap-8 max-w-3xl">
            {[
              ...
              <div key={i} className="reveal" style={{ animationDelay: `${i * 120}ms` }}>
                ...
                <div className="mt-1 text-xs sm:text-sm uppercase tracking-[0.15em] text-white/65">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="border-t border-white/10 bg-black/10">
          <div className="container-page py-4 flex flex-wrap gap-x-8 gap-y-2 text-xs text-white/70">
            <span className="inline-flex items-center gap-2"><ShieldCheck size={14}/> Profiles verified by office</span>
            <span className="inline-flex items-center gap-2"><Building2 size={14}/> Registered Trust · Reg. 42/1952</span>
            <span className="inline-flex items-center gap-2"><Users size={14}/> Hindu & Christian Nadar families</span>
          </div>
        </div>
      </section>
      ...
      <section className="container-page py-20">
        <div className="flex items-end justify-between gap-6 flex-wrap">
          <div>
            <div className="chip">Our work</div>
            <h2 className="mt-3 font-display text-3xl md:text-4xl text-primary-deep max-w-xl">{t.services}</h2>
            <p className="mt-2 text-muted-foreground max-w-md">{t.servicesSub}</p>
          </div>
          <Link to="/about" className="btn-ghost">Learn about the sangam <ArrowRight size={14}/></Link>
        </div>

        <div className="mt-10 grid sm:grid-cols-2 lg:grid-cols-3 gap-px bg-border rounded-2xl overflow-hidden border border-border">
          {services.map((s) => (
            <Link to={s.to} key={s.title} className="group bg-card p-7 hover:bg-primary-soft/60 transition-colors">
              <div className="leaf-mark"><s.icon size={16} /></div>
      ...
      <section className="bg-surface border-y border-border">
        <div className="container-page py-20">
          <div className="flex items-end justify-between gap-6 flex-wrap">
            <div>
              <div className="chip">Matrimony</div>
              <h2 className="mt-3 font-display text-3xl md:text-4xl text-primary-deep">{t.featured}</h2>
              <p className="mt-2 text-muted-foreground max-w-md">{t.featuredSub}</p>
            </div>
            <Link to="/matrimony" className="btn-ghost">View all profiles <ArrowRight size={14}/></Link>
          </div>
            ...
            {PROFILES.slice(0, 3).map((p) => (
              <article key={p.id} className="card-flat overflow-hidden">
                <div className="relative h-56 bg-gradient-to-br from-primary-soft to-[#d8ebe0] flex items-center justify-center">
                  <div className="w-24 h-24 rounded-full bg-primary-deep text-white font-display text-4xl flex items-center justify-center backdrop-blur">
                    {p.initial}
                  </div>
                  <span className="absolute top-3 left-3 chip !bg-white">{p.id}</span>
                  <span className="absolute top-3 right-3 chip !bg-white">Verified</span>
                </div>
                <div className="p-5">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-y-1.5 text-xs text-muted-foreground">
                    <span>Rasi</span><span className="text-foreground">{p.rasi}</span>
                    ...
                    <span>Religion</span><span className="text-foreground">{p.religion} Nadar</span>
                  </div>
                  <button className="mt-5 btn-ghost w-full justify-center" onClick={() => toast.success("Interest sent — sangam will contact you.")}>
                    Express Interest
                  </button>
                </div>
              </article>
      ...
      <section className="container-page py-20">
        <div className="flex items-end justify-between gap-6 flex-wrap">
          ...
          <Link to="/events" className="btn-ghost">All events <ArrowRight size={14}/></Link>
        </div>
  ...
  return (
    <section className="bg-primary-soft/60 border-t border-border">
      <div className="container-page py-20 grid lg:grid-cols-2 gap-12">
        <div>
          <div className="chip">Visit us</div>
          <h2 className="mt-3 font-display text-3xl md:text-4xl text-primary-deep">{t.contact}</h2>
          <p className="mt-2 text-muted-foreground max-w-md">{t.contactSub}</p>
        ...
        <form
          onSubmit={(e) => { e.preventDefault(); toast.success("Enquiry received — we'll be in touch."); setForm({ name: "", phone: "", message: "" }); }}
          className="card-flat p-6 sm:p-8 bg-white"
        >
          <h3 className="font-display text-xl text-primary-deep">Send an enquiry</h3>
